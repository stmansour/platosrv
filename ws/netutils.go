...
func SvcUserTypeDown(w http.ResponseWriter, r *http.Request, d *ServiceData) {
	...
	url := fmt.Sprintf("%sv1/peopletd?%s", db.Pdb.Config.AuthNHost, url.QueryEscape(q))
...
// 	//----------------------------------------------------------------------
// 	url := fmt.Sprintf("%sv1/people/0/%d", db.Pdb.Config.AuthNHost, uid)
...
func GetUserListInfo(uids []int64, sess *session.Session) ([]UserInfo, error) {
	...
	// the business portion of the URL is ignored.  We snap it to 0
	//----------------------------------------------------------------------
	url := fmt.Sprintf("%sv1/people/0", db.Pdb.Config.AuthNHost)
