...
if [ "x${PSRVBIN}" = "x" ]; then
	...
	echo "PSRVBIN was pre-set to:  \"${PSRVBIN}\""
fi

TREPORT="${PSRVBIN}/../../test/testreport.txt"
PSRV="${PSRVBIN}/platosrv"
GOLD="./gold"
...
startPsrv() {
	if [ ${MANAGESERVER} -eq 1 ]; then
		stopPSRV
		cmd="${PSRV} > ${PSRVBIN}/platosrvlog 2>&1 &"
		echo "${cmd}"
		${PSRV} > ${PSRVBIN}/platosrvlog 2>&1 &
		sleep 1
		rm -f platosrvlog
		ln -s ${PSRVBIN}/platosrvlog slog
	fi
}
...
doCypressUITest () {
	...
	echo "STOPPING platosrv SERVER in cypress UI automated testing" # finally
	stopRentRollServer
